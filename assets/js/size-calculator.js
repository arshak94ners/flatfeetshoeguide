...
(function () {
  ...
  function convert(system, rawValue) {
    ...
    if (isNaN(value) || value <= 0) {
      return { error: "Please enter a valid size, e.g. 9 or 9.5." };
    }

    var plausible = PLAUSIBLE[system];
    if (!plausible) {
      return { error: "Unknown sizing system." };
    }
    if (value < plausible[0] || value > plausible[1]) {
      return {
        error:
          SYSTEM_LABELS[system] +
          " sizes are generally between " +
          formatSize(plausible[0]) +
          " and " +
          formatSize(plausible[1]) +
          " for adults. Double-check the value and system you selected."
      };
    }
  ...
  function renderResult(container, system, result) {
    ...
    var items = [
      ...
      { key: "usM", label: "US Men's", value: formatSize(result.usM) },
      { key: "usW", label: "US Women's", value: formatSize(result.usW) },
      { key: "uk", label: "UK", value: formatSize(result.uk) },
      { key: "cm", label: "CM", value: formatSize(result.cm) }
    ];
    ...
    if (result.beyondTable) {
      html +=
        '<p class="calc-error">That size is past the edge of our EU 35–50 reference table, so this is the closest boundary estimate, not a precise interpolation. Please check the manufacturer\'s own size chart directly.</p>';
    } else {
      html +=
        '<p class="calc-note">Estimated by interpolating the reference table above. Fit varies by brand and last — always confirm against the manufacturer’s official size chart before buying, especially for EU 42–45 and half sizes.</p>';
    }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var PLACEHOLDERS = { eu: "e.g. 42", usM: "e.g. 9.5", usW: "e.g. 11", uk: "e.g. 8.5", cm: "e.g. 27" };
