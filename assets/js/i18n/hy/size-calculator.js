...
(function () {
  ...
  function convert(system, rawValue) {
    ...
    if (isNaN(value) || value <= 0) {
      return { error: "Մուտքագրեք վավեր չափս, օրինակ՝ 9 կամ 9.5։" };
    }

    var plausible = PLAUSIBLE[system];
    if (!plausible) {
      return { error: "Անհայտ չափսի համակարգ։" };
    }
    if (value < plausible[0] || value > plausible[1]) {
      return {
        error:
          SYSTEM_LABELS[system] +
          " չափսերն ընդհանուր առմամբ գտնվում են " +
          formatSize(plausible[0]) +
          "-ի և " +
          formatSize(plausible[1]) +
          "-ի միջև՝ մեծահասակների համար։ Ստուգեք ընտրված արժեքն ու համակարգը։"
      };
    }
  ...
  function renderResult(container, system, result) {
    ...
    var items = [
      ...
      { key: "usM", label: "US տղամարդու", value: formatSize(result.usM) },
      { key: "usW", label: "US կանացի", value: formatSize(result.usW) },
      { key: "uk", label: "UK", value: formatSize(result.uk) },
      { key: "cm", label: "ՍՄ", value: formatSize(result.cm) }
    ];
    ...
    if (result.beyondTable) {
      html +=
        '<p class="calc-error">Այս չափսը դուրս է մեր EU 35–50 տիրույթի աղյուսակի սահմաններից, ուստի սա մոտավոր սահմանային գնահատական է, ոչ թե ճշգրիտ ինտերպոլացիա։ Ստուգեք արտադրողի սեփական չափսերի աղյուսակը։</p>';
    } else {
      html +=
        '<p class="calc-note">Գնահատված է վերևի հղման աղյուսակի ինտերպոլացիայի միջոցով։ Հարմարվածությունը տարբերվում է ըստ բրենդի և կաղապարի &mdash; միշտ ստուգեք արտադրողի պաշտոնական չափսերի աղյուսակի հետ գնումից առաջ, հատկապես EU 42–45 և կես չափսերի համար։</p>';
    }
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    var PLACEHOLDERS = { eu: "օր.՝ 42", usM: "օր.՝ 9.5", usW: "օր.՝ 11", uk: "օր.՝ 8.5", cm: "օր.՝ 27" };
