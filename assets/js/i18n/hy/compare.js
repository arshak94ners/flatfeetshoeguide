...
(function () {
  ...
  var METRIC_LABELS = {
    ...
    cushioning: "Ամորտիզացիա",
    wideFoot: "Հարմարություն լայն ոտքերի համար",
    value: "Արժեք",
    durability: "Դիմացկունություն"
  };
  ...
  function groupedOptions(selectedKey) {
    ...
    var html = '<option value="">Ընտրեք ապրանք&hellip;</option>';
  ...
  function renderPicker(root, state) {
    ...
    [0, 1, 2].forEach(function (i) {
      html +=
        ...
        '<label for="compare-slot-' + i + '">Կոշիկ ' + (i + 1) + (i === 2 ? " (ընտրովի)" : "") + "</label>" +
  ...
  function renderTable(state) {
    ...
    if (keys.length < 2) {
      tableRoot.innerHTML = '<p class="muted">Համեմատելու համար ընտրեք վերևից առնվազն երկու ապրանք։</p>';
      return;
    }
    ...
    var overallRow =
      "<tr><td><strong>Ընդհանուր միավոր</strong></td>" +
      keys.map(function (key) { return "<td><strong>" + ffsgOverallScore(key).toFixed(1) + " / 10</strong></td>"; }).join("") +
      "</tr>";

    var html =
      '<div class="table-wrap"><table class="compare"><thead><tr><th scope="col">Հատկանիշ</th>' + head + "</tr></thead><tbody>" +
      row("Կատեգորիա", function (p) { return p.categoryLabel; }) +
      row("Լավագույնն է հետևյալի համար", function (p) { return p.bestFor; }) +
      row("Աջակցության տեսակ", function (p) { return p.supportType; }) +
      row("Դրոփ / stack", function (p) { return p.drop; }) +
      row("Լայնության տարբերակներ", function (p) { return p.widthOptions; }) +
      row("Գնային միջակայք", function (p) { return ffsgPriceLabel(p.priceTier); }) +
      overallRow +
